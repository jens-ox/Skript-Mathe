...
\begin{bla}{Wichtige Funktionen: Potenzen von $x$}
  Die wohl am häufigsten verwendeten Funktionen sind die \emph{Potenzen} von $x$, also $x^2, x^6$ und so weiter. Sie lassen sich in zwei Sorten unterteilen:
  \begin{itemize}
    ...
    \item \textbf{Exponent ungerade}: Ist der Exponent ungerade, so sieht der Graph wie bei einem geraden Exponenten aus, nur, dass die beiden `Äste' des Graphen in unterschiedliche Richtungen zeigen:
    \begin{marginfigure}
      \begin{tikzpicture}[
          scale=0.7,
          thick,
